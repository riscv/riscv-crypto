...
inline uint32_t decsm (uint32_t rs1, uint32_t rs2, int bs) {
    uint32_t tx;
    tx   = _saes_v3_decs(rs1,   0, bs);
    return _saes_v3_decm(tx , rs2, bs);
}
...
void aes_ecb_decrypt (
    ...
    while (1) {
        ...
        u0 = decsm(t0, u0, 0); //  AES decryption round, 16 instr
        u0 = decsm(t3, u0, 1);
        u0 = decsm(t2, u0, 2);
        u0 = decsm(t1, u0, 3);
        ...
        t0 = decsm(u0, t0, 0); //  AES decryption round, 16 instr
        t0 = decsm(u3, t0, 1);
        t0 = decsm(u2, t0, 2);
        t0 = decsm(u1, t0, 3);
...
static void aes_dec_invmc(
    ...
    for (i = 0; i < len; i++) {
        ...
        x = decsm(t, 0, 0);        // Just want inv MixCol()
        x = decsm(t, x, 1);        //
        x = decsm(t, x, 2);        //
        x = decsm(t, x, 3);        //
