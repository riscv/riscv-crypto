...
| XLEN-1:31 | FAIL  | FAIL=0 if seed is a valid output, FAIL=1 otherwise.
|     30:16 |       | Reserved for future use.
...
the low 16 bits of positive values having a random seed word.

The FAIL and non-fail conditions may convey additional information
in extended (non-baseline) implementations in bits 30:16; for example
indications about failure modes (for FAIL=1) or immediate availability of
further entropy bits (for FAIL=0).
...
conditioned randomness due to hardware limitations on smaller, low-powered implementations. Cryptographic conditioners such as AES would be too
large to implement solely to serve GetRandom -- and non-cryptographic
conditioners too wasteful on input entropy. Hence you should not use the
output directly but poll twice the amount of required entropy and use that
to seed a cryptographic DRBG. Implementors are urged to use AES or SHA
instruction extensions to build such a DRBG.
...
*   **Entropy:** A minimum requirement is that 128 bits of "full
    ...
    output sequence via (vetted) cryptographic conditioning algorithm
...
An acceptable random number generator initialization sequence would obtain 512
bits (32 successful calls) of GetEntropy output and hash it with a suitable
mechanism such as SHA2/3-384/512 or SHAKE256 <<FIPS180>> <<FIPS202>>
...
satisfying the criteria above (>8 bits of true entropy), or FAIL
(high bit set).
...
cryptographic standards <<NISTPQC>>, which generally do not require a QRNG.
