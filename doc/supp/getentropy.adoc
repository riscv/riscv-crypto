...
since all such interaction mechanisms with its "state" can be potentially 
used in attacks.
...
instruction should not pass on DRBG output unless the entropy content 
and secret state of that generator is considered to be sufficient for 
high-security applications (more than 256 bits).
