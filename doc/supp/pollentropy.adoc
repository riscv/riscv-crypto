...
<<SP800-90B>> compliant random number generators can be built using PollEntropy
and other RISC-V cryptographic instructions (notably AES and SHA). PollEntropy
...
| XLEN-1:32 |      | [RV64] Sign-extend bit 31 (for arithmetic shift etc).
...
| 0 | 0 | BIST | Built-In Self-Test in progress following a systemwide reset.
...
Implementors should not make any bits available in `seed` unless the state
is ES16.
...
*   **BIST** indicates a Built-In Self-Test and additional "on-demand"
    statistical testing is performed. We suggest that it should not last
    more than a few hundred milliseconds after reset. Any runtime
    unavailability caused by continuous testing after the initial BIST
    should be indicated with WAIT instead.
...
*   **WAIT** means that a sufficient amount of entropy is not yet available.
    ...
    If polling in a loop, we suggest WFI (wait for interrupt) call before
    then next poll.

*   **ES16** indicates success; the low bits `rd[15:0]` will have 16 bits
    of randomness which is guaranteed to have at least 8 bits true entropy
    regardless of implementation. For example, `0x4000ABCD` is a valid
    output, with `0xABCD` containing the `seed`.
...
*   **DEAD** is an unrecoverable self-test error. This may indicate a
    ...
    appropriate response, especially in dedicated security devices.


NOTE:   To guarantee that no sensitive data is read twice and
that different callers don't get correlated output, hardware
implementations should execute *wipe-on-read* on the randomness pathway
during each successful read. For the same reasons, "uncooked" randomness
should bever be made visible via PollEntropy.
...
viewpoint and implemented only if they are truly necessary (and can be
formally modeled). This also applies to the raw noise source.
Section 3.2.2, <<SP800-90B>> specifically states that "The noise source
state shall be protected from adversarial knowledge or influence to the
__greatest extent possible__."
...
rates of output entropy are required. Hence a
caller should not use the output directly but poll twice the amount of
required entropy, condition it cryptographically (e.g. by hashing twice
the amount needed), and use that to seed a cryptographic DRBG.
Implementors are urged to use AES or SHA instruction extensions to build
conditioners and DRBGs.


====    Baseline output requirements for PollEntropy(0)

*   **Entropy:** Each 16-bit output sample (`seed`) has at least 8 bits of
    independent, unpredictable true randomness (entropy). Technically this
...
is never sufficient). The generator should be able to support "LEVEL 5"
post-quantum cryptography standards <<NISTPQC>>. These requirements
will be refined to reflect changes in relevant standards.
...
*   **On-demand testing** is usually invoked via resetting, rebooting, or
    ...
    program should be able to invoke them in the field (the term is a
    throw-back to an earlier era when random number generators were not
...
*   **Continuous tests** often maintain a state (such as counters) related to
    ...
    handle some statistical tests as they see fit.
...
Some higher-level TRNG module (scope of certification and evaluation)
may consist of the hardware circuit together with a suitable driver
that implements the required health checks, cryptographic conditioning,
etc. The delineation of the ISA component is as follows:
...
SP 800-90B GetNoise(), if needed, can be implemented via hardware debug
interface or vendor-specific CSR (out of scope) and HealthTest() is
interfaced via system reset and other some other out-of-band mechanism.
...
-   Cryptographic conditioning, e.g. SHA2/HMAC, AES/CMAC, AES/CBC-MAC as
    ...
    PollEntropy output into n bits of "full entropy".
...
driver development. PollEntropy has an argument parameter that can be used
