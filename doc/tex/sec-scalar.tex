...
uint32_t lut4_hi_lo (uint32_t rs1, uint32_t rs2, bool hi) {
    uint64_t result;
    for(int i = 0; i < 32; i += 4) {
        uint8_t idx = (rs1 >> i) & 0x7;
        bool    hb  = (rs1 >> i) & 0x8;
        if(hi ^ hb) { result |= ((rs2 >> (4*idx) & 0xF) << i; }
    }
}
...
\item[RV32] 
    Splits the lookup step into two stages. The {\tt lo} instruction
    updates nibbles in the destination with the looked-up value
    iff the index is less than eight. The {\tt hi} version does
    the same for index values greater than/equal to eight.
    The results can then be or'd together.
    
    \begin{lstlisting}
lut4lo :
    for i = 0..7 : rd.4[i] =  (rs1.4[i]&0x8) ? 0 : rs2.4[rs1.4[i]&0x7]
lut4hi:
    for i = 0..7 : rd.4[i] = !(rs1.4[i]&0x8) ? 0 : rs2.4[rs1.4[i]&0x7]
    \end{lstlisting}
    
    \begin{lstlisting}[style=ASM]
lut4lo  t0, a0, a1      // a0 = indexes, a1 = lo 2 bits of results.
lut4hi  t1, a0, a2      // a0 = indexes, a2 = hi 2 bits of results.
