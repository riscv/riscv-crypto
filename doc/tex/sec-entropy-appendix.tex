...
\subsection{Specific Rationale and Considerations}
    ...
    Figure \ref{fig:esstate-tikz} illustrates operational state
...
\begin{figure}[tb]
    ...
    \caption{Normally the operational state alternates between WAIT
        ...
        has been polled. DEAD is an unrecoverable state. BIST
        (Built-in Self-Test) only occurs after reset or to signal
        a non-fatal self-test alarm (if reached after WAIT or ES16).}
...
\label{sec:cont-tests}
    ...
    if such signaling is necessary for some reason related to certification.
