...
\begin{itemize}
    ...
    is a valid \verb|ES16| status output, with \verb|0xABCD| being the
    \verb|seed| value.
...
\label{sec:es-polling}
    ...
    no need to pool after a \verb|DEAD| state.
...
\label{sec:security-controls}
    ...
    material. In almost all cases a hardware entropy source must implement
    appropriate \emph{security controls} to guarantee unpredictability,
    prevent leakage, detect attacks, and to deny adversarial control over
    the entropy output or ts generation mechanism.
    ...
    the entropy source will enter a no-output state. We define that an
    online or noise alarm has occurred when the entropy source is in
    WAIT mode for one second or more. Critical failures can result in DEAD
    state immediately.
