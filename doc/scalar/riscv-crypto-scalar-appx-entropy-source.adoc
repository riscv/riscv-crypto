...
===	A Design and Self-Certification Checklist
...
*	*Architectural Tests.* Verify RISC-V requirements in this specification,
	such as that `sentropy` ES16 seed words can only be read once, and that
	the state transitions (<<crypto_scalar_sentropy>>,
	<<crypto_scalar_es_getnoise>>) and
	access control (<<crypto_scalar_es_access>>) are correctly implemented.
	The scope of RISC-V architectural tests are those behaviors that 
	are independent of the entropy source details (a smoke test ES module
...
*	*Technical justification for entropy.* This may take the form of a
	...
	process and also not observable or guessable. A complete physical model is
	typically not necessary; research literature can be cited. For example,
	good ring oscillator noise is not merely random-looking but
	derives some amount of physical entropy from local, spontaneously
	occurring Johnson-Nyquist thermal noise cite:[Sa21].
*	*Design Review.* An entropy source is more than a noise source, and has
	features such as health tests (<<crypto_scalar_es_security_controls>>),
	...
	used. Simpler, non-official checklist tables are available:
...
*	*Conditioner and Health Test Design.* One needs to show that the
	conditioner and health test components work appropriately to either
	meet the ES16 output requirements of <<crypto_scalar_es_req>>
	or fail (BIST / DEAD).
...
have a relatively small area (just a few thousand NAND2 gate equivalent) as an
Entropy Source circuit itself does not need to contain components such as AES.
The availability of such IP modules is increasing as in 2021, CMVP is
introducing a separate Entropy Source Validation Scope. This allows entropy
source validations to be re-used for different (FIPS 140-3) modules that
contain ("consume") them.
...
cite:[nist:fips:202,nist:fips:180:4] ), and the AES-based CBC-MAC
...
In order for shared drivers to be able to accommodate the test interface
in a secure fashion, we suggest that it is implemented as a custom, M-mode
CSR, denoted here as `mnoise`. This is to facilitate the use of shared
RISC-V self-certification scripts for entropy sources.
...
the secret signing key. Even a temporary lapse in ES16 security may break
