...
*	*Experimental Tests.* The raw noise source is subjected to entropy
	estimation (H~original~)
    defined in NIST 800-90B, Section 3.1.3 cite:[TuBaKe:18].
...
around an `S` or `HS` mode component in relation to Sensitive Security
Parameter (SSP) flows, which is
...
detecting security issues – hence the deprevent weak crypto keys from being generated).
...
Other contents and behavior of the register can be
