...
*	*Architectural Tests.* Verify RISC-V requirements in this specification,
	such as the entropy source state transitions (<<crypto_scalar_sentropy>>)
	and access control (<<crypto_scalar_es_access>>). The scope of RISC-V
	architectural tests are those behaviors that are independent of the
	entropy source details (a smoke test ES module may be sufficient).
