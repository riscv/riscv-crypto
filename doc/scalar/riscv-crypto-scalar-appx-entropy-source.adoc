...
*	*Architectural Tests.* Verify RISC-V requirements in this specification;
	`sentropy` ES16 seed words can only be read once, and state transitions
	(<<crypto_scalar_sentropy>> and <<crypto_scalar_es_getnoise>>) and
...
*	*Technical justification for entropy.* This may take the form of a
	...
	process, and also not directly observable. A complete physical
	model is not necessary; research literature can be cited.
	For example, good ring oscillator noise is not merely random-looking
	because it derives an amount of physical entropy from local, spontaneously
