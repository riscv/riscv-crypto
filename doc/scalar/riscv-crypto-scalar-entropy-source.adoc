...
Note that though it is not shown in the diagram, there is no requirement for
a state change after a `WAIT` or `ES16` state.
====
...
The `sentropy` CSR is not available to User Mode Software. The table
...
It is guaranteed that access to `sentropy` will make `seed` entropy
