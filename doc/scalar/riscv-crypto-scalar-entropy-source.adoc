...
currently considered to be part of the sampling ("digitisation")
...
We define that a non-critical alarm is signalled if the entropy source
...
externally available, and it must be zeroised periodically or upon
...
production use. Its contents and behaviour must be interpreted in the
...
mentropy) is read-write. We define a pseudo-instruction for reading it:
...
The behaviour of other input and output bits is left to the vendor.
