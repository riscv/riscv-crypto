...
processor features (e.g. PMP enhancements).footnote:[
More information on the `mseccfg` CSR may be found in the
link:https://docs.google.com/document/d/1Mh_aiHYxemL0umN3GTTw8vsbmzHZ_nxZXgjgOUzbvc8/edit#heading=h.hw4f0hsg3t3[PMP Enhancements for memory access and execution prevention on Machine mode] proposal.
]
If `Zkr` is _not_ implemented, the `SKES` bit must be hardwired
to zero.
If `Zkr` is the only implemented feature requiring `mseccfg`,
then all bits other than `SKES` must be hardwired to zero, or
otherwise set according to the relevant specifications for a not implemented
feature.
