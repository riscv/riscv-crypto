...
These instructions enable a single key, specified as a single element group in `vs2`, to be
applied to each element group of a vector of element groups in `vd`.
