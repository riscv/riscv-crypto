...
to the other groups using an instruction such as _vrgather_.
...
[source,pseudocode]
--
function clause execute (VAESESKR2(vd, rcon, vs2)) = {
 if((unsigned(rnd) > 14) | unsigned(rnd) = 0) then {
  ...
  foreach (i from eg_start to eg_len-1) {
      ...
      let previous : bits(128) = aes_256_reverse_key_schedule(rcon, current, previous);
