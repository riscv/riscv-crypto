...
Synopsis::
Vector SHA-256 2 rounds of compression.

Mnemonic::
vsha256ca.vv vd, vs2, vs1 +
vsha256cb.vv vd, vs2, vs1
...
We might want to replace the suffixes with "h" and "l" for high and low
...
This instruction has an EGW=128, that is, it operates on 128-bit elements.
...
This instruction treats `EGW=128`, regardless of `vtype.vsew` and requires that
