...
function. Starting with the final 2 round keys, it generates each of the earlier round keys and performs each of the rounds.
...
[source,sail]
--
function clause execute (VAES128D(vs1, vd, vv)) = {
  assert(VLEN>=128);
