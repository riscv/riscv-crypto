...
====
- `SEW`=32: 4 rounds of SHA-256 message schedule expansion are performed (`zvknha` and `zvknhb`)
- `SEW`=64: 4 rounds of SHA-512 message schedule expansion are performed (`zvkhnb`)

Eleven of the last 16 message schedule `SEW`-sized words are read in from `vd` (oldest), `vs2`
and `vs1` (most recent). 
 A subset of the last 16 message-schedule words are consumed and
the next 4 message-schedule words are produced.

The initial 16 words of the message schedule come directly from the message block; these must
must be endian swapped by software from RISC-V's native little-endian to SHA-2's big endian format.
