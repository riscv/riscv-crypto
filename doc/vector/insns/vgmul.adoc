...
Description:: 
A single multiplication of the GHASH~H~ algorithm is performed.

This instruction treats all of the inputs and outputs as 128-bit polynomials and 
performs operations over GF[2].
It produces (Y~i+1~) by multiplying  (over GF(2^128^)) the current partial
hash (Y~i~) by the Hash Subkey (H).
