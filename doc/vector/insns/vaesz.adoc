...
There is only a `.vs` form of the instruction. Element group 0 of the single register `vs2` holds a
single element group that is used
as the round key for all of the element groups of the register group `vd` which holds the round state.
