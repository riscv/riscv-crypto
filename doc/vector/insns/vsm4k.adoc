...
Description::
This instruction implements 4 rounds of the SM4 Key Expansion.

It produces four 32-bit round keys as defined in SM4 (see
...
The Round group number (`rnd`) comes from `uimm[3:0]`; the bits in `uimm[4:3]` are ignored.
...
[NOTE]
====
The initial round keys are generated before this instruction by XORing the 128-bit encryption key with a so-called system parameter: FK[0:3]
====
