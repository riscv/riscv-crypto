...
Description::
A carry-less multiply is performed and the low half of the product is returned. This operation is sometimes also referred to as a polynomial multiply.

All active elements in the `vs2` vector register are multiplied by 
either a vector register group `vs1`, or a scalar
integer register `rs1`.
For the scalar integer register, the operand is zero extended or truncated
to 64 bits as appropriate.
The least significant EEW bits of the product are returned.
