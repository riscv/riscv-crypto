...
The current state of eight 32-bit words is read in as an element group. Eight 32-bit message words
are read in, although only four of them are used. Based on the round group be being processed
(as specified in rnds), the next state of eight 32-bit words is generated.
...
[NOTE]
====
To align with the SM3 specification, we internally byte swap the inputs from big-endian to
little-endian, we perform the operations in little-endian, 
and then we byte swap the output back to big endian.
====
