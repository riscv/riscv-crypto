...
function clause execute (VSM4R(vd, vs2)) = {
  ...
  foreach (i from eg_start to eg_len-1) {
    ...
    // 1st round
    B = (X1 ^ X2 ^ X3 ^ rk0)
    S = SUB_BYTES_SM4(B); 1 s-box per byte
    let Y0 : bits(32) = X0 ^ S ^ ROTL(S,2)^ ROTL(S,10)^ ROTL(S,18) ^ ROTL(S,24)
    // 2nd round
    B = (X2 ^ X3 ^ X4 ^ rk1)
    S = SUB_BYTES_SM4(B); 1 s-box per byte
    Y1= X1 ^ S ^ ROTL(S,2)^ ROTL(S,10)^ ROTL(S,18) ^ ROTL(S,24)
    //3rd round
    B = (X3 ^ Y0 ^ X1 ^ rk2)
    S = SUB_BYTES_SM4(B); 1 s-box per byte
    Y2= X2 ^ S ^ ROTL(S,2)^ ROTL(S,10)^ ROTL(S,18) ^ ROTL(S,24)
    //4th round
    B = (Y0 ^ Y1 ^ Y2 ^ rk3)
    S = SUB_BYTES_SM4(B); 1 s-box per byte
    Y3= X3 ^ S ^ ROTL(S,2)^ ROTL(S,10)^ ROTL(S,18) ^ ROTL(S,24)
