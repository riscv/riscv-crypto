...
Within each element group, the next round key is generated word by word from the
...
word of the current round key as well as a round constant.
...
The round number is used to specify a round constant which is used in generating the first
round key word.
