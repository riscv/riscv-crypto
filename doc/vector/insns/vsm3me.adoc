...
The input words are swapped from big endian to little endian.
The sixteen most recent 32-bit message words are read in as two
eight-element groups.
The next eight 32-bit message words are generated and returned in an eight-element group.
The output words are swapped from little endian to big endian.
...
and then we byte swap the output from little endian back to big endian.
====
