[[insns-vclmaccgh, Vector Carryless Multiply over Galois-Field 2^128]]
= vclmaccgh.vv
...
[source,pseudocode]
--
function clause execute (VCLMACCGH(vs2, vs1, vd)) = {
  ...
  foreach (i from egstart to elementgroups) {
    ...
    let prod[255:0]   = clmul_128(Y, H); // MSB is always 0
