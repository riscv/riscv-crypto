...
Extension specification is listed here.

All cryptographic instructions defined in this Vector Crypto specification (i.e., <<zvkns>>,
<<zvknh[ab]>>, <<Zvkg>>, <<Zvksed>> and <<zvksh>> but not <<zvkb>>) shall be executed with
data independent execution latency.
That is, the latency of an instruction's execution shall not vary based on the values
of its inputs. This behavior is the same as described as described in the `Zkt` extension for
