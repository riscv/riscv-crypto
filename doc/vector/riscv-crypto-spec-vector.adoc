...
The <<zvknh,Zvknhb>> and <<zvbc>> Vector Crypto Extensions require a Zve64x base,
...
// `SEW`=64 and therefore cannot be implemented on a Zve32* base.

// While the <<zvbb>> extension can be built on a Zve32* base, the `vclmul[h]` instructions will not be
...
<<zvknh,Zvknh[ab]>>, <<Zvkg>>, <<Zvksed>> and <<zvksh>> but _not_ <<zvbb>>) shall be executed with
...
(including <<zvbb>>) to be executed with data-independent execution latency.
...
<<<
include::insns/vrevb.adoc[leveloffset=+2]
<<<
...
<<<
include::insns/vrev.adoc[leveloffset=+2]
<<<
...
include::./riscv-crypto-vector-inst-table-zvbb.adoc[]
