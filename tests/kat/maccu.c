...
/* Known answer tests for the MACCU instruction. Used to check that the
...
__maccu(unsigned int rs1, unsigned int rs2, unsigned int rs3) {
    uint32_t rd2, rd1;
    __asm__ ("maccu (%0,%1), %2, %3, %4" 
        : "=r"(rd2), "=r"(rd1)
        : "r"(rs1), "r"(rs2), "r"(rs3)
    );
    uint64_t result = rd2;
...
int test_maccu() {
    ...
    for(int i = 0; i < 10; i ++) {
        ...
        result = __maccu(rs1,rs2,rs3);

        expect = ((((uint64_t)rs1) << 32) | rs2) + rs3;
        
        // TODO: Why does printf ignore the "l" in "%lx" ?
        printf("maccu (%x || %x) + %x, got %lx, expect, %lx\n",
